...
public class SellLtcHandler implements BaseHandler {
    ...
    public HttpResponseBuilder handleRequest(ParsedRequest request) {
        ...
        try {
            ...
            Double ltcAmountToSell = requestData.get("ltcAmount");
            Double ltcPrice = requestData.get("ltcPrice");

            if (ltcAmountToSell <= 0 || ltcPrice <= 0) {
                // Handle invalid BTC amount or price
                RestApiAppResponse<TransactionDto> response = new RestApiAppResponse<>(false, null, "Invalid LTC amount or price to sell");
            ...
            double totalCost = ltcAmountToSell * ltcPrice;

            // Check if the user has sufficient funds
            if (user.getLTC() < ltcAmountToSell) {
                RestApiAppResponse<TransactionDto> response = new RestApiAppResponse<>(false, null, "Insufficient LTC");
            ...
            sellTransaction.setCryptoType("LTC");  // Assuming BTC is being bought
            sellTransaction.setCryptoPrice(ltcPrice);
            ...
            RestApiAppResponse<TransactionDto> response = new RestApiAppResponse<>(true, null, "LTC sale successful");
